  ...
  return (
    <div>
            ...
            <div className="sm:text-center lg:text-left">
              <h1 className="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
                <span className="block xl:inline">
                  Chat with {name} on Workdock
                </span>
              </h1>
              <p className="mt-3 text-black sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-2xl lg:mx-0 font-semi">
                Workdock replaces Zoom for unscheduled calls
              </p>
              <p className="mt-3 text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
                You can see teammates, like {name}, as avatars directly on your
                MacOS dock and start a voice call or screen-share with one
                click.
              </p>
              {!copied && (
                <div className="mt-5 sm:mt-8 sm:justify-center lg:justify-start md:w-auto lg:w-96 invisible md:visible">
                  <div className="rounded-md shadow">
                    <div
                      onClick={() => {
                        clipboard.copy(
                          `https://workdock.app/?invitedby=${fromEmail}`
                        );
                        setCopied(true);
                      }}
              ...
              {copied && (
                <iframe
                  src={`workdock://${fromEmail.replace(/@/g, "__")}`}
                  width="0"
                  height="0"
                ></iframe>
              )}
              {copied && (
                <div>
                  <div className="mt-3 font-bold text-base text-black sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
                    To accept {name}'s invite, download and sign in below if you
                  ...
                  <div className="mt-5 sm:mt-8 sm:justify-center lg:justify-start md:w-auto lg:w-96 invisible md:visible">
                    ...
                    <a
                      className="mt-3 cursor-pointer text-sm text-gray-500 underline"
                      href={`workdock://${fromEmail.replace(/@/g, "__")}`}
                    >
                      Didn't work? Click here to launch Workdock
                    </a>
                  </div>
                </div>
              )}
                ...
                <div className="rounded-md shadow">
                  <span className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-gray-800 bg-gray-300 md:py-4 md:text-lg md:px-7">
                    <span className="mt-1 text-center">
                      Use your Mac to download Workdock
                    </span>
                  </span>
                </div>
              </div>
      ...
      <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2 lg:h-full invisible md:visible mt-5">
        <img
          className="h-full w-full object-cover object-left sm:h-72 md:h-96 lg:w-full lg:h-full"
          src="workdock.png"
          alt=""
        />
      </div>
